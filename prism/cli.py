...
def cmd_run(args):
    """Run PRISM pipeline on input data (user-facing entry point)."""
...
def cmd_inspect(args):
    ...
    from prism.input_loader import load_input, detect_data_characteristics
    ...
    print(f"\nRun pipeline:")
    print(f"  prism run {input_path}")
    print(f"  prism run {input_path} --atlas")
...
def _print_summary(output_dir: Path, manifest: dict):
    ...
    print(f"\n{'='*60}")
    print(f"  PRISM Pipeline Complete")
    print(f"{'='*60}")
    print(f"  Output: {output_dir}")
    ...
    print(f"\n  View results:")
    print(f"    prism explore {output_dir}")
    print(f"{'='*60}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog='prism',
        description='PRISM — Domain-agnostic dynamical systems analysis',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Quick start:
    prism run sensor_data.csv              # Run core pipeline
    prism run sensor_data.csv --atlas      # Run with full atlas
    prism inspect sensor_data.csv          # Analyze input data
    prism explore ./prism_output/          # Launch visualization

Pipeline commands:
    prism validate /path/to/data
    prism signal /path/to/manifest.yaml
    prism status /path/to/data
    prism atlas /path/to/data
        """,
    )
    ...
    run_parser = subparsers.add_parser(
        'run',
        help='Run PRISM pipeline on input data (CSV, parquet, or directory)',
    )
    ...
    run_parser.add_argument(
        '--output', '-o', default='./prism_output',
        help='Output directory for parquet files (default: ./prism_output)',
    )
    run_parser.add_argument(
